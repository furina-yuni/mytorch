...
SUMMARY = {
    ...
    "current_device": "현재 Python thread에서 선택된 CUDA 장치 번호를 반환합니다.",
    "synchronize": "지정 CUDA 장치에 예약된 작업이 모두 끝날 때까지 기다립니다.",
    "memory_allocated": "CuPy memory pool에서 현재 Tensor 배열이 사용하는 byte 수를 반환합니다.",
    "memory_reserved": "CuPy memory pool이 CUDA runtime으로부터 확보한 전체 byte 수를 반환합니다.",
    "max_memory_allocated": "peak 통계 초기화 이후 관찰된 최대 배열 사용량을 반환합니다.",
    "max_memory_reserved": "peak 통계 초기화 이후 관찰된 최대 pool 확보량을 반환합니다.",
    "reset_peak_memory_stats": "현재 사용량을 기준으로 지정 장치의 peak memory counter를 초기화합니다.",
    "mem_get_info": "CUDA runtime이 보고하는 물리 장치의 여유·전체 memory byte를 반환합니다.",
    "memory_stats": "현재·peak pool 사용량과 물리 장치 여유량을 dict로 반환합니다.",
    "memory_summary": "CUDA memory 통계를 사람이 읽기 좋은 여러 줄 문자열로 반환합니다.",
    "empty_cache": "CuPy pool에서 사용하지 않는 device·pinned memory block을 반환합니다.",
    ...
    "IterableDataset": "길이를 미리 정하지 않고 sample을 순서대로 생성하는 streaming dataset 기반입니다.",
    "ImageFolder": "클래스 이름별 하위 폴더의 RGB 이미지를 label index와 함께 읽습니다.",
    "CSVDataset": "CSV의 선택한 열을 numeric feature와 target sample로 읽습니다.",
    "NpyDataset": "입력과 target NPY 배열을 선택적으로 memory-map해 sample 단위로 제공합니다.",
    "Sampler": "Dataset에서 읽을 sample index의 순서를 정의하는 기반 클래스입니다.",
    "SequentialSampler": "Dataset index를 처음부터 끝까지 순서대로 반환합니다.",
    "RandomSampler": "복원 가능한 난수 상태로 Dataset index 순서를 섞거나 복원 추출합니다.",
    "SubsetRandomSampler": "지정한 일부 index만 매 iteration 무작위 순서로 반환합니다.",
    "WeightedRandomSampler": "sample별 가중치에 비례해 index를 추출합니다.",
    "BatchSampler": "다른 Sampler의 index를 고정 크기 mini-batch 목록으로 묶습니다.",
    "Compose": "여러 CPU transform을 등록 순서대로 하나의 sample에 적용합니다.",
    "ToArray": "PIL 또는 NumPy 이미지를 CHW 형식의 연속 NumPy 배열로 변환합니다.",
    "Resize": "이미지를 지정한 높이와 너비로 보간해 변경합니다.",
    "CenterCrop": "이미지 중앙에서 지정 크기를 잘라내고 부족한 영역은 padding합니다.",
    "RandomCrop": "재현 가능한 sample RNG로 이미지의 무작위 위치를 crop합니다.",
    "RandomHorizontalFlip": "지정 확률로 이미지의 좌우를 뒤집습니다.",
    "Normalize": "CHW 배열에 channel별 (input - mean) / std를 적용합니다.",
    "autocast": "대상 CUDA 연산을 FP16으로 실행하고 수치 민감 연산은 FP32로 유지하는 context입니다.",
    "GradScaler": "loss scale을 동적으로 조절하고 비정상 gradient의 optimizer step을 건너뜁니다.",
    "is_autocast_enabled": "현재 실행 문맥에서 AMP autocast가 활성화됐는지 반환합니다.",
    "get_autocast_dtype": "현재 autocast가 행렬곱 계열에 사용하는 dtype을 반환합니다.",
    ...
    "inference_mode": "평가 전용 연산의 계산 그래프와 autograd bookkeeping을 비활성화합니다.",
    "is_inference_mode_enabled": "현재 실행 문맥이 inference Tensor를 생성하는지 반환합니다.",
    "grad": "leaf grad에 누적하지 않고 출력에서 입력으로의 vector-Jacobian product를 반환합니다.",
    "detect_anomaly": "backward 오류와 non-finite gradient에 연산 이름과 forward 위치를 추가합니다.",
    "set_detect_anomaly": "현재 문맥의 anomaly detection 상태와 non-finite 검사를 설정합니다.",
    "is_anomaly_enabled": "현재 anomaly detection 활성화 상태를 반환합니다.",
    ...
    "clip_grad_norm_": "Parameter gradient의 전체 p-norm이 상한을 넘지 않도록 같은 비율로 줄입니다.",
    "clip_grad_value_": "각 Parameter gradient 원소를 지정 절댓값 범위로 제한합니다.",
    "profile": "CUDA event를 사용해 context 안의 MyTorch 연산 시간과 입력 shape를 기록합니다.",
    "FunctionEvent": "한 번 실행된 GPU 연산의 이름, 장치, CUDA 시간과 입력 shape 기록입니다.",
    "FunctionEventAvg": "같은 이름의 GPU 연산 호출 횟수와 총·평균 CUDA 시간 집계입니다.",
    "get_env_info": "MyTorch, Python, CuPy, CUDA runtime과 GPU 환경을 안전하게 수집합니다.",
    "format_env_info": "환경 정보 dict를 버그 보고에 붙이기 좋은 문자열로 변환합니다.",
}
...
OPTIMIZERS = {
    ...
    "LRScheduler": "Optimizer parameter group의 learning rate와 checkpoint 위치를 관리합니다.",
    "StepLR": "고정 epoch 간격마다 learning rate에 gamma를 곱합니다.",
    "MultiStepLR": "지정 milestone을 지날 때마다 learning rate에 gamma를 곱합니다.",
    "ExponentialLR": "매 step마다 지수적으로 감소하는 learning rate를 계산합니다.",
    "LinearLR": "정해진 step 동안 시작 factor에서 종료 factor까지 선형 보간합니다.",
    "CosineAnnealingLR": "cosine 곡선으로 base learning rate에서 eta_min까지 변화시킵니다.",
    "CosineAnnealingWarmRestarts": "주기적으로 base learning rate로 돌아가는 cosine schedule입니다.",
    "SequentialLR": "여러 scheduler를 milestone 경계에서 순서대로 전환합니다.",
    "OneCycleLR": "batch 단위 learning rate 상승과 하강을 한 cycle로 수행합니다.",
    "ReduceLROnPlateau": "검증 metric이 개선되지 않으면 learning rate를 감소시킵니다.",
}
