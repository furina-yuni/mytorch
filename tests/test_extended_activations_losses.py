...
def _finite_gradient(function, source: np.ndarray, step: float = 1e-5) -> np.ndarray:
    ...
    for index in np.ndindex(source.shape):
        ...
        minus[index] -= step
        gradient[index] = (
            function(mt.tensor(plus)).item() - function(mt.tensor(minus)).item()
        ) / (2 * step)
    return gradient
...
def test_smooth_activation_gradients(function) -> None:
    ...
    expected = _finite_gradient(lambda item: function(item).sum(), source)
